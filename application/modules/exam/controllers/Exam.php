    ...
    function create($id = '') {
        if ($_POST) {            
            ...
            if (count($is_record_present)) {
                ...
                {
                   /** Start Quiz Create         */
                $quiz_insert_id = $this->Quiz_model->insert(array(
                'title' => $_POST['exam_name'],               
                'department_id' => $_POST['degree'],
                'branch_id' => $_POST['course'],
                'batch_id' => $_POST['batch'],
                'semester_id' => $_POST['semester'],                                
                'total_marks' => $_POST['total_marks'],
                'start_date' => date('Y-m-d', strtotime($_POST['date'])),
                'end_date' => date('Y-m-d', strtotime($_POST['end_date_time'])),
                'status' => '0',
                'exam_id'=>$insert_id));
                $quiz = array("quiz_id"=>$quiz_insert_id);
               
                
               $this->db->update("exam_manager",$quiz,array("em_id"=>$insert_id));
                }
                    /** end Quiz Create */
            ...
            if ($this->form_validation->run('exam_insert_update') != FALSE) {
                ...
                if($exam_mode=="mcq")   
                {
                    ...
                    if($exam->quiz_id=="0")
                    {
                        ...
                        $this->db->update("exam_manager",$quiz,array("em_id"=>$param2));
